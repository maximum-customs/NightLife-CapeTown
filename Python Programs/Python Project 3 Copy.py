...
class Sircle:
    ...
    def circle_displacement(self):
        self.centre[0] += self.velocity * cos(radians(self.direction))
        self.centre[1] += self.velocity * sin(radians(self.direction))
    ...
    def circle_collision(self, circleB):
        ...
        while ChaA < -180:
            ChaA += 360
        #ChaA_final = ChaA * ( circleB.mass / self.mass )/2
        ChaA_final = ChaA/2
        Sdf = Sd + ChaA_final
        #ChaA = Sdf - Sd
...
circle2 = Sircle([80,80,100], [250,300], 50, 5, 170, 10)
